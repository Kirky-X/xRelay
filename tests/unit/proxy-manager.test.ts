...
describe("Proxy Manager - 初始化", () => {
  ...
  it("应该正确初始化代理管理器（内存模式）", async () => {
    // 重新导入以获取新的模块实例
    const { initProxyManager, isUsingDatabase } =
  ...
  it("初始化失败时应该降级到内存模式", async () => {
    // 模拟初始化过程中的错误
    const { fetchAllProxies } = await import("../../src/proxy-fetcher.js");
    vi.mocked(fetchAllProxies).mockRejectedValueOnce(
...
describe("Proxy Manager - 代理获取", () => {
  ...
  it("应该返回可用代理", async () => {
    const { initProxyManager, getAvailableProxy } =
      await import("../../src/proxy-manager.js");
  ...
  it("应该返回多个代理", async () => {
    const { initProxyManager, getMultipleProxies } =
      await import("../../src/proxy-manager.js");
  ...
  it("获取代理前应该确保已初始化", async () => {
    ...
    const { getAvailableProxy } = await import("../../src/proxy-manager.js");
...
describe("Proxy Manager - 失败报告", () => {
  ...
  it("应该正确报告代理失败", async () => {
    const { initProxyManager, reportProxyFailed, getPoolStatus } =
      await import("../../src/proxy-manager.js");
  ...
  it("报告失败后应该更新断路器状态", async () => {
    const { initProxyManager, reportProxyFailed, getCircuitBreakerStatus } =
      await import("../../src/proxy-manager.js");
  ...
  it("失败报告应该从内存池中移除代理", async () => {
    const { initProxyManager, getMultipleProxies, reportProxyFailed } =
      await import("../../src/proxy-manager.js");
...
describe("Proxy Manager - 断路器", () => {
  ...
  it("应该跟踪断路器状态", async () => {
    const { initProxyManager, getCircuitBreakerStatus } =
      await import("../../src/proxy-manager.js");
  ...
  it("断路器打开后代理应该不可用", async () => {
    const { initProxyManager, reportProxyFailed, getAvailableProxy } =
      await import("../../src/proxy-manager.js");
    ...
    // 如果返回代理，不应该是断路器打开的那个
    if (availableProxy) {
      expect(`${availableProxy.ip}:${availableProxy.port}`).not.toBe(
        `${proxy.ip}:${proxy.port}`,
      );
    }
  });
...
describe("Proxy Manager - 池状态", () => {
  ...
  it("应该返回正确的池状态", async () => {
    const { initProxyManager, getPoolStatus } =
      await import("../../src/proxy-manager.js");
  ...
  it("应该返回正确的代理统计信息", async () => {
    const { initProxyManager, getProxyStats } =
      await import("../../src/proxy-manager.js");
...
describe("Proxy Manager - 手动刷新", () => {
  ...
  it("应该支持手动刷新代理池", async () => {
    const { initProxyManager, manualRefresh, getPoolStatus } =
      await import("../../src/proxy-manager.js");
    ...
    expect(statusAfter.refreshCount).toBeGreaterThanOrEqual(
      statusBefore.refreshCount,
    );
  });
});
...
describe("Proxy Manager - 边界情况", () => {
  ...
  it("报告不存在的代理失败不应该抛出错误", async () => {
    const { initProxyManager, reportProxyFailed } =
      await import("../../src/proxy-manager.js");
  ...
  it("报告不存在的代理成功不应该抛出错误", async () => {
    const { initProxyManager, reportProxySuccess } =
      await import("../../src/proxy-manager.js");
