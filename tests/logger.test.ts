...
describe('Logger', () => {
  ...
  describe('info', () => {
    it('调用 console.log，输出 JSON 含 level=info、message、context（service/version/environment）', () => {
      ...
      expect(entry.context.version).toBe('0.2.0');
  ...
  describe('logRequest', () => {
    ...
    it('URL 含 query string 时遮蔽为 ?[REDACTED]', () => {
      logger.logRequest('req-1', 'GET', 'https://example.com/path?token=secret&foo=bar', 200, 10);
      const entry = JSON.parse(logSpy.mock.calls[0][0] as string);
      expect(entry.context.url).toBe('https://example.com/path?[REDACTED]');
    });

    it('URL 无 query 时保留 protocol/host/pathname', () => {
      logger.logRequest('req-2', 'GET', 'https://example.com/path/sub', 200, 10);
      const entry = JSON.parse(logSpy.mock.calls[0][0] as string);
