...
export async function fetchAllProxies(): Promise<ProxyInfo[]> {
  ...
  if (cachedProxies.length > 0 && (now - lastFetchTime) < CACHE_DURATION) {
    console.log(`[ProxyFetcher] 使用缓存的代理列表，共 ${cachedProxies.length} 个`);
  ...
  const fetchPromises = PROXY_SOURCES.map(async (source) => {
    try {
      ...
      console.log(`[ProxyFetcher] 从 ${source.name} 获取了 ${proxies.length} 个代理`);
      return proxies;
    } catch (error) {
 ...
 * 从单个源获取代理
 */
async function fetchFromSource(source: typeof PROXY_SOURCES[0]): Promise<ProxyInfo[]> {
